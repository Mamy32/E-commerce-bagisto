...
<div class="panel-side journal-scroll grid max-h-[1320px] min-w-[342px] max-w-[380px] grid-cols-[1fr] gap-8 overflow-y-auto overflow-x-hidden max-xl:min-w-[270px] max-md:max-w-full max-sm:min-w-0 max-md:gap-5">
    ...
    <div class="grid grid-cols-[auto_1fr] items-center gap-4 rounded-xl border border-zinc-200 px-5 py-[25px] max-md:py-2.5">
        ...
        <div 
            class="flex flex-col justify-between min-w-0"
            v-pre
        >
            <p class="text-2xl font-medium truncate max-md:text-xl" title="Hello! {{ $customer->first_name }}"> 
                Hello! {{ $customer->first_name }}
            </p>

            <p class="no-underline text-zinc-500 truncate max-md:text-sm" title="{{ $customer->email }}">
                {{ $customer->email }}
            </p>
        </div>
    </div>
